...
export async function GET(
  ...
  try {
    ...
    // Convertiamo il genere in minuscolo
    const lowerCaseGenere = genere.toLowerCase();

    console.log("🔍 Genere richiesto:", genere);

    // Trovare l'ID del genere corrispondente
    const genre = await Genre.findOne({
      name: lowerCaseGenere,
    }).select("_id");
    if (!genre) {
    ...
    let query: Record<string, any> = { "genres.id": genre._id };
    ...
    if (filters.topics && filters.topics.length > 0) {
      ...
      if (topics.length > 0) {
        // Aggiorniamo per la nuova struttura
